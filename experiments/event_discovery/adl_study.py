...
# definition source: https://www.adl.org/resources/report/audit-antisemitic-incidents-2023
        ...
        for forced in [True, 
            ...
            for definition in [True, 
                ...
                if (definition and shot == 1) or (shot == 1 and (not forced)):
                    continue
                pth = f"adl_results/{shorthand}/{shot}_{str(forced).lower()}_{definition}.csv"
                if os.path.exists(pth):
                    ...
                    shape = (0, 0)
                for i, (text, date, city, state) in enumerate(tups):
                    if i < shape[0]:
                        continue
...
df2 = pd.read_csv("../../data/TOXIGEN/control_adl.csv").sample(df.shape[0], random_state = 0)
df2.to_csv(f"../../data/TOXIGEN/control_adl_{df.shape[0]}.csv")
